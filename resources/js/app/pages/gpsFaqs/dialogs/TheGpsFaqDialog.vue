<template>
    <BaseEditDialog
        form-id="gps-faq-form"
        :record="record"
        record-type="GPS FAQ"
        endpoint="api/gps_faqs"
    ...
    >
        <template #content="{getErrors, handleSubmit, didSubmit}">
            <form id="gps-faq-form" @submit.prevent="handleSubmit" class="flex flex-col gap-y-4">
                                ...
                                <TabPanel v-for="lang in languagesStore.languages" :value="lang.id">
                                    <div class="flex flex-col gap-y-4">
                                        <BaseInputContainer
                                            ...
                                            <InputText maxlength="255" v-model="form.languages[lang.code].question"/>
                                        </BaseInputContainer>

                                        <BaseInputContainer
                                            :show-errors="didSubmit"
                                            label="Answer"
                                            :errors="getErrors(`languages.${lang.code}.answer`)">
...
async function recordMapper(data) {
    ...
    try {
        startDialogLoading({
            blockUI: true,
            message: 'Fetching GPS FAQ information...'
        })

        if (props.record?.id) {
            const response = await window.axios.get(`api/gps_faqs/${props.record.id}`)
