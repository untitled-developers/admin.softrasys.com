<template>
    ...
    >
        <template #content="{getErrors, handleSubmit, didSubmit}">
            <form id="partner-form" @submit.prevent="handleSubmit" class="flex flex-col gap-y-4">
...
async function recordMapper(data) {
    ...
    try {
        startDialogLoading({
            blockUI: true,
            message: 'Fetching partners information...'
        })

        if (props.record?.id) {
            const response = await window.axios.get(`api/partners/${props.record.id}`)
