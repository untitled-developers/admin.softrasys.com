<template>
    <BaseEditDialog
        form-id="gps-industry-form"
        :record="record"
        record-type="GPS Industry"
    ...
    >
        <template #content="{getErrors, handleSubmit, didSubmit}">
            <form id="gps-industry-form" @submit.prevent="handleSubmit" class="flex flex-col gap-y-4">
...
async function recordMapper(data) {
    ...
    try {
        startDialogLoading({
            blockUI: true,
            message: 'Fetching GPS industry information...'
        })

        if (props.record?.id) {
            const response = await window.axios.get(`api/gps_industries/${props.record.id}`)
