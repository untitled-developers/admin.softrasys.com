<template>
    <BasePageContent>
        <BaseCrudTable :edit-dialog="TheGpsIndustryDialog"
                       endpoint="api/gps_industries"
                       ref="crudTable">
            <template #columns>
                <Column field="id" header="ID" :sortable="true"></Column>
                <Column header="Icon">
                    <template #body="{data}">
                        <img v-if="data.blob_url" :src="data.blob_url" class="w-10 h-10 object-contain"/>
                    </template>
                </Column>
...
import {ref} from "vue";
import TheGpsIndustryDialog from "@/js/app/pages/gpsIndustries/dialogs/TheGpsIndustryDialog.vue";
...
async function handleToggleHidden(value, record) {
    ...
    try {
        await window.axios.put(`api/gps_industries/${record.id}/toggleHidden`);
    } catch (e) {
