<template>
    ...
    >
        <template #content="{getErrors, handleSubmit, didSubmit}">
            <form id="industry-form" @submit.prevent="handleSubmit" class="flex flex-col gap-y-4">
...
async function recordMapper(data) {
    ...
    try {
        startDialogLoading({
            blockUI: true,
            message: 'Fetching Industries information...'
        })

        if (props.record?.id) {
            const response = await window.axios.get(`api/industries/${props.record.id}`)
