...
async function markAsRead() {
    if (props.record && props.record.is_read === 0) {
        try {
            await window.axios.put(`api/resellerForms/${props.record.id}/toggleRead`);
            emit('submit');
        } catch (error) {
            console.error('Error marking reseller form as read:', error);
            alertError('Failed to mark reseller form as read');
        }
    }
}
