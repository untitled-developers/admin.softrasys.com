...
<BasePageContent>
        <BaseCrudTable
            endpoint="api/resellerForms"
            :with-add="false"
            with-clear-filters
            edit-mode="cell"
            :filters="filters"
            :edit-dialog="TheResellerFormInfoDialog"
...
import TheResellerFormInfoDialog from "@/js/app/pages/forms/resellerForm/dialogs/TheResellerFormInfoDialog.vue";
...
function handleCellEditComplete(event, field, data) {
    ...
    isUpdatingData = true;
    axios.post(`api/resellerForms/${data.id}/updateStatus`, post)
        .then(() => {
            fetchTableData();
            stopRowLoading(data);
        })
