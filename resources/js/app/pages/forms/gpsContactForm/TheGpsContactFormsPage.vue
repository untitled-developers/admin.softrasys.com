...
<BasePageContent>
        <BaseCrudTable
            endpoint="api/gpsContactForms"
            :with-add="false"
            with-clear-filters
            edit-mode="cell"
            :filters="filters"
            :edit-dialog="TheGpsContactFormInfoDialog"
...
import TheGpsContactFormInfoDialog from "@/js/app/pages/forms/gpsContactForm/dialogs/TheGpsContactFormInfoDialog.vue";
...
function handleCellEditComplete(event, field, data) {
    ...
    isUpdatingData = true;
    axios.post(`api/gpsContactForms/${data.id}/updateStatus`, post)
        .then(() => {
            fetchTableData();
            stopRowLoading(data);
        })
